...
hexo.extend.helper.register('doc_sidebar', function (className) {
    var type = this.page.canonical_path.split('/');
    type.pop();
    var sidebar = (function (sidebar, type) {
        for (var i in type) {
            if ('undefined' == typeof sidebar[type[i]]) {
                return false;
            }
            sidebar = sidebar[type[i]];
        }
        return sidebar;
    })(this.site.data.sidebar, type);
...
hexo.extend.helper.register('canonical_path_for_nav', function () {
    ...
    if (startsWith(path, 'fastd/') || startsWith(path, 'components/')) {
        return path;
    } else {
        return '';
    }
